...
export default function Restaurant() {
  ...
  useEffect(() => {
    const q = query(
      ...
      where("listing.category", "==", "restaurant")
    );
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      const data = querySnapshot.docs.map((doc) => {
        ...
        return {
          ...
          menu: d.menu || [],
          desc: d.desc,
          likes: d.listing.likes || 0,      // default 0 if not exist
          dislikes: d.listing.dislikes || 0 // default 0 if not exist
        };
      });
  ...
  useEffect(() => {
    const loadVotes = async () => {
      const savedVotes = await AsyncStorage.getItem('restaurantVotes');
      if (savedVotes) {
  ...
  useEffect(() => {
    AsyncStorage.setItem('restaurantVotes', JSON.stringify(votes));
  }, [votes]);

// ✅ Handle voting with anti-spam protection
const handleVote = async (index, type) => {
  const itemId = restaurantData[index].id;

  // 🚫 Ignore if Firestore update is still pending
  ...
  try {
    ...
    // 🔄 Double-check local storage to prevent double vote
    // const storedVotes = JSON.parse(await AsyncStorage.getItem('restaurantVotes') || '{}');
  ...
  return (
    ...
    <ScrollView contentContainerStyle={styles.listContainer}>
      {filteredRestaurant.length === 0 ? (
        // ✅ Show "no data" icon + message when list is empty
        <View style={styles.noDataContainer}>
          <Ionicons name="information-circle-outline" size={30} color="#888" />
          <Text style={styles.noDataText}>
            {searchQuery
              ? 'No results found for your search.'
              : 'No restaurant listings available.'}
          </Text>
        </View>
      ) : (
