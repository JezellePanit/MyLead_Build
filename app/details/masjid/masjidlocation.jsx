...
export default function MasjidLocation() {
  ...
  const [masjidCoords, setMasjidCoords] = useState(null);
  ...
  useEffect(() => {
    if (masjidCoords && mapRef.current) {
      mapRef.current.animateToRegion({
        latitude: masjidCoords.latitude,
        longitude: masjidCoords.longitude,
        latitudeDelta: 0.01,
        longitudeDelta: 0.01,
      }, 1000);
    }
  ...
  const decodePolyline = useCallback((t) =>
    polyline.decode(t).map(([lat, lng]) => ({ latitude: lat, longitude: lng }))
  , []);

  // Simplify polyline
  const simplifyPolyline = useCallback((coords, factor = 5) =>
    coords.filter((_, i) => i % factor === 0)
  , []);

  // Reset routeType when mode changes
  const handleModeChange = useCallback((newMode) => {
    setMode(newMode);
    setRouteType("shortest");
  }, []);

  // Fetch route whenever mode, routeType, userLocation, or destination changes
  useEffect(() => {
    const fetchRoute = async () => {
      if (!userLocation || !masjidCoords) return;
      setLoading(true);

      const origin = `${userLocation.latitude},${userLocation.longitude}`;
      const destination = `${masjidCoords.latitude},${masjidCoords.longitude}`;
    ...
    fetchRoute();
  }, [mode, routeType, userLocation, masjidCoords, decodePolyline, simplifyPolyline]);

  // Memoized Mode Button
  const ModeButton = React.memo(({ keyMode, selectedMode, onPress, label, icon }) => (
    <TouchableOpacity
      key={`mode-${keyMode}`} // ✅ fixed unique key
      ...
      <View style={styles.transportContainer}>
        {transportModes.map(({ key, label, icon }) => (
          <ModeButton
            ...
            keyMode={key} // ✅ fixed with unique key
            selectedMode={mode}
...
const styles = StyleSheet.create({
  ...
  modeButton: { flex: 1, marginHorizontal: 3, paddingVertical: 10, borderRadius: 8, backgroundColor: "#e0e0e0", alignItems: "center" },
  ...
  infoContainer: { position: "absolute", bottom: 110, left: 10, right: 10, flexDirection: "row", justifyContent: "space-around", backgroundColor: "#e0e0e0", padding: 8, borderRadius: 6 },
