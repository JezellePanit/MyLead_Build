//firebase (not needed since static location, but kept consistent import layout)
...
// ✅ Hardcoded NCMF Coordinates
const NCMF_COORDS = {
  latitude: 16.4262,
  longitude: 120.5896,
};

export default function NCMFLocation() {
  ...
  // ✅ Animate map to NCMF marker
  useEffect(() => {
    if (NCMF_COORDS && mapRef.current) {
      mapRef.current.animateToRegion(
        {
          latitude: NCMF_COORDS.latitude,
          longitude: NCMF_COORDS.longitude,
          latitudeDelta: 0.01,
          longitudeDelta: 0.01,
        },
        1000
  ...
  const simplifyPolyline = useCallback(
    (coords, factor = 5) => coords.filter((_, i) => i % factor === 0),
    []
  );

  // ✅ Handle mode change (reset to shortest)
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      ...
      const destination = `${NCMF_COORDS.latitude},${NCMF_COORDS.longitude}`;
        ...
        if (data.routes && data.routes.length > 0) {
          ...
          if (routeType === "shortest") {
            selectedRoute = data.routes.reduce((prev, curr) =>
              prev.legs[0].distance.value < curr.legs[0].distance.value
                ? prev
                : curr
            );
          ...
          setDistance(
            (selectedRoute.legs[0].distance.value / 1000).toFixed(1)
          );
          setDuration(
            (selectedRoute.legs[0].duration.value / 60).toFixed(1)
          );
    ...
    fetchRoute();
  }, [mode, routeType, userLocation, decodePolyline, simplifyPolyline]);

  // ✅ Memoized Mode Button
  const ModeButton = React.memo(
    ({ keyMode, selectedMode, onPress, label, icon }) => (
      <TouchableOpacity
        key={`mode-${keyMode}`}
        style={[
          styles.modeButton,
          selectedMode === keyMode && styles.selectedMode,
        ]}
        onPress={onPress}
      >
        <View style={styles.modeContent}>
          <MaterialIcons
            name={icon}
            size={20}
            color={selectedMode === keyMode ? "#fff" : "#333"}
          />
          <Text
            style={[
              styles.modeText,
              selectedMode === keyMode && styles.selectedText,
            ]}
          >
            {label}
          </Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.replace("tabs/locationpage/locate")}
        >
      ...
      >
        <Marker
          coordinate={NCMF_COORDS}
          title="NCMF North Luzon"
          pinColor="purple"
        />
            ...
            {["standard", "hybrid", "terrain"].map((key) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  mapType === key && styles.activeOption,
                ]}
                onPress={() => {
                  setMapType(key);
          ...
          <View style={styles.dropdown}>
            {routeOptions.map((opt) => (
              <TouchableOpacity
                ...
                style={[
                  styles.dropdownOption,
                  routeType === opt.toLowerCase() && styles.activeOption,
                ]}
                onPress={() => {
              ...
              >
                <Text
                  style={[
                    styles.dropdownText,
                    routeType === opt.toLowerCase() && styles.activeText,
                  ]}
                >
                  {opt}
                </Text>
...
const styles = StyleSheet.create({
  ...
  layerButton: { backgroundColor: "white", padding: 8, elevation: 4 },
  dropdown: {
    marginTop: 5,
