...
export default function RestaurantLocation() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setRestaurantName(listing.listing_name || "Restaurant");

        if (listing.coordinates) {
          const lat = Number(listing.coordinates._lat);
          const lng = Number(listing.coordinates._long);
          if (!isNaN(lat) && !isNaN(lng)) {
            setRestaurantCoords({ latitude: lat, longitude: lng });
          }
        }
      }
    });
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      if (!userLocation || !restaurantCoords) return;
      setLoading(true);

      const origin = `${userLocation.latitude},${userLocation.longitude}`;
      const destination = `${restaurantCoords.latitude},${restaurantCoords.longitude}`;
    ...
    fetchRoute();
  }, [mode, routeType, userLocation, restaurantCoords, decodePolyline, simplifyPolyline]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.backIcon} onPress={() => router.replace("tabs/homepage/restaurant")}>
      ...
      <View style={styles.transportContainer}>
        {transportModes.map(({ key, label, icon }) => (
          <ModeButton
            key={key} // fix unique key warning
            keyMode={key}
            selectedMode={mode}
