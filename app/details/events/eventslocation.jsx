...
export default function EventsLocation() {
  ...
  // 🔹 Fetch Firestore document for event
  useEffect(() => {
    if (!id) return;
    ...
    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setEventName(campaign.title || "Event");

        if (campaign.coordinates) {
          const lat = Number(campaign.coordinates._lat);
          const lng = Number(campaign.coordinates._long);
          if (!isNaN(lat) && !isNaN(lng)) {
            setEventCoords({ latitude: lat, longitude: lng });
          }
        }
      }
    });
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      if (!userLocation || !eventCoords) return;
      setLoading(true);

      const origin = `${userLocation.latitude},${userLocation.longitude}`;
      const destination = `${eventCoords.latitude},${eventCoords.longitude}`;
    ...
    fetchRoute();
  }, [mode, routeType, userLocation, eventCoords, decodePolyline, simplifyPolyline]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.backIcon} onPress={() => router.replace("tabs/homepage/events")}>
      ...
      <MapView
        ...
        mapType={mapType}
      >
        {eventCoords && <Marker coordinate={eventCoords} title={eventName} pinColor="purple" />}
