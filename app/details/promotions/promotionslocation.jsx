...
export default function PromotionsLocation() {
  ...
  const [promoCoords, setPromoCoords] = useState(null);
  ...
  // 🔹 Fetch Firestore document for promotion
  useEffect(() => {
    if (!id) return;
    const docRef = doc(db, "campaigns_db", id); // using campaigns_db with category "promotion"
    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.data();
        const promo = data.campaign || {};
        setPromoName(promo.title || "Promotion");

        if (promo.coordinates) {
          const lat = Number(promo.coordinates._lat);
          const lng = Number(promo.coordinates._long);
          if (!isNaN(lat) && !isNaN(lng)) {
            setPromoCoords({ latitude: lat, longitude: lng });
          }
        }
      }
    });
  ...
  useEffect(() => {
    if (promoCoords && mapRef.current) {
      mapRef.current.animateToRegion(
        {
          latitude: promoCoords.latitude,
          longitude: promoCoords.longitude,
          latitudeDelta: 0.01,
          longitudeDelta: 0.01,
        },
        1000
  ...
  useEffect(() => {
    const fetchRoute = async () => {
      if (!userLocation || !promoCoords) return;
      setLoading(true);

      const origin = `${userLocation.latitude},${userLocation.longitude}`;
      const destination = `${promoCoords.latitude},${promoCoords.longitude}`;
    ...
    fetchRoute();
  }, [mode, routeType, userLocation, promoCoords, decodePolyline, simplifyPolyline]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.backIcon} onPress={() => router.replace("tabs/homepage/promotions")}>
      ...
      <MapView
        ...
        mapType={mapType}
      >
        {promoCoords && <Marker coordinate={promoCoords} title={promoName} pinColor="purple" />}
